<template>
  ...
  <!--  用完就删除这个文件。-->
  <v-col v-for="w in formlist" :key="w.name" class="ma-0 pa-4" cols="12" xs="3" sm="3" md="3" lg="3" xl="3">
        ...
        <v-col cols="8" class="ma-0 pa-0">
          ...
          <div v-if="n.type=='addtextfield'">
            <v-container class="ma-0 pa-0" v-for="q in n.valuelist" :key="q.index">
              <v-row dense no-gutters class="ma-0 pa-0">
                <v-col cols="9">
                  <v-text-field class="ma-0 pa-0" v-model="q.value" required outlined dense
                                style="font-size:15px;width:100%;transform:scale(0.75,0.75);"></v-text-field>
                </v-col>
                <v-col cols="3">
          ...
          <div v-if="n.type=='select'">
            <v-select :items="n.items" v-model="n.value" :rules="n.rules" label="请选择" solo flat dense outlined
                      style="font-size:15px;width:100%;transform:scale(0.75,0.75);"></v-select>
          </div>
          <v-textarea v-model="n.value" required outlined flat dense background-color="white"
                      style="font-size:15px;width:100%;transform:scale(0.75,0.75);" auto-grow
                      v-if="n.type=='textarea'"></v-textarea>
          <input type="file" id="upload" ref="upload" @change="changeimg" accept=".jpg, .jpeg, .png"
                 v-if="n.type=='pictureupload'" style="display:block;width:90%;font-size:10px;margin-top:10px;">
          <img :src="n.src" width="70%" alt="" v-if="n.type=='picture'"/>
          <v-text-field class="ma-0 pa-0" v-model="n.value" required outlined dense :rules="n.rules"
                        :disabled="n.disabled"
                        style="font-size:15px;width:100%;transform:scale(0.75,0.75);" v-if="n.type=='singleline'"
          ></v-text-field>
          <v-radio-group :rules="n.rules" v-model="n.value" v-if="n.type=='radio'" row
                         style="width:100%;transform:scale(0.75,0.75);" class="ma-2 pa-0" :disabled="n.disabled">
            <v-radio v-for="i in n.radiochoice" :key="i" :label="i" :value="i" style="font-size:10px;"></v-radio>
          </v-radio-group>
          <v-menu ref="n.menu" v-model="n.menu" :close-on-content-click="false" transition="scale-transition" offset-y
                  min-width="290px" v-if="n.type=='timeselect'">
            <template v-slot:activator="{ on, attrs }">
              <v-text-field v-model="n.value" prepend-icon="event" readonly required outlined dense v-bind="attrs"
                            v-on="on" style="font-size:20px;width:100%;transform:scale(0.75,0.75);"></v-text-field>
            </template>
            <v-date-picker v-model="n.value" no-title scrollable :max="n.maxdate">
              <v-spacer></v-spacer>
              <v-btn text color="primary" @click="n.menu = false">取消</v-btn>
              <v-btn text color="primary" @click="n.menu = false">确定</v-btn>
            </v-date-picker>
          </v-menu>
        </v-col>
      </v-row>
